...
with wandb.init(config=config,job_type="model_evaluation") as run:
    ...
    #tasks = ["arc_easy","hellaswag","drop","sciq"]
    tasks = ["drop","sciq"]
...
#    table_contents.append(results["results"]["squad2"]["f1"])
    ...
    run.log({
        ...
        "Validation Responses": get_completion_responses_batch(prompts, config.BASE_MODEL, production_model, staging_model, tokenizer),
        "Model Latencies": get_latency_df(prompts, 10, config.BASE_MODEL, production_model, staging_model, tokenizer)
    })
    run.log_code()
