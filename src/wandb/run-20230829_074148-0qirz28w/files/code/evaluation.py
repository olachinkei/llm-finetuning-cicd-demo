...
def get_completion_responses_batch(prompts, base_llm,production_model, stag_model, tokenizer):
    # Get completions for each model in batches
    original_completions = get_huggingface_completion_batch(prompts, base_llm,tokenizer)
    stagaing_completions = get_huggingface_completion_batch(prompts, staging_model, tokenizer)
    production_completions = get_huggingface_completion_batch(prompts, production_model, tokenizer)
    responses = []
    for opt, ft in zip(original_completions, stagaing_completions,production_completions):
        responses.append({
            "Original": original_completions,
            "Staging (finetuned)": production_completions,
            "Production (finetuned)": production_completions
        })
    df = pd.DataFrame(responses)
    df.insert(0, "prompt", prompts)
    return df

def get_huggingface_completion_batch(prompts, model, tokenizer):
...
def get_latency_df(prompts, num_prompts, base_llm, production_model, staging_model, tokenizer):
  ...
  original_time = timeit.timeit(lambda: get_huggingface_completion_batch(prompts, base_llm, tokenizer), number=1)
  production_time = timeit.timeit(lambda: get_huggingface_completion_batch(prompts, production_model, tokenizer), number=1)
  staging_time = timeit.timeit(lambda: get_huggingface_completion_batch(prompts, staging_model,tokenizer), number=1)
...
with wandb.init(config=config,job_type="model_evaluation") as run:
    ...
    ## staging model
    staging_model = wandb.use_artifact(f'{os.environ["WANDB_ENTITY"]}/model-registry/{config.MODEL_REGISTRY}:staging')
    ...
    ## production model
    production_model = wandb.use_artifact(f'{os.environ["WANDB_ENTITY"]}/model-registry/{config.MODEL_REGISTRY}:production')
    production_path = production_model.download()
    production_model = PeftModel.from_pretrained(base_llm, staging_path,torch_dtype=torch.float16)
...
#    table_contents.append(results["results"]["drop"]["f1"])
    ...
    run.log({
        ...
        "Validation Responses": get_completion_responses_batch(prompts, base_llm,production_model, staging_model, tokenizer),
        "Model Latencies": get_latency_df(prompts, num_prompts, base_llm, production_model, staging_model, tokenizer)
    })
    run.log_code()
    ...
    pg = wr.PanelGrid(
        runsets=runsets,
        panels=[
            wr.WeavePanelSummaryTable("Evaluation metrics", layout={'w': 24, 'h': 12}),
        ])
