...
def get_completion_responses_batch(prompts, base_llm,production_model, stag_model, tokenizer):
    ...
    responses = []
    for ori, sta, in zip(original_completions, stagaing_completions,production_completions):
        responses.append({
            "Original": ori,
...
#    table_contents.append(results["results"]["drop"]["f1"])
    ...
    run.log({
        ...
        "Model Latencies": get_latency_df(prompts, 10, base_llm, production_model, staging_model, tokenizer)
    })
    run.log_code()
