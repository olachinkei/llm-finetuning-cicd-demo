...
train_data.to_pandas().to_parquet(f"review_data/train/{INITIAL_UPLOAD_DATE}.parquet", index=False)
valid_data.to_pandas().to_parquet(f"review_data/valid/{INITIAL_UPLOAD_DATE}.parquet", index=False)
test_data.to_pandas().to_parquet(f"review_data/test/{INITIAL_UPLOAD_DATE}.parquet", index=False)

with wandb.init() as run:
    artifact = wandb.Artifact('reviews', type='dataset')
    artifact.add_dir("review_data/")
    run.log_artifact(artifact, aliases=["production"])

for file in glob.glob("review_data/*.parquet"):
    os.remove(file)
